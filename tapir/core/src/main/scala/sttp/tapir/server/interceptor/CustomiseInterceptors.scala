...
/** Allows customising the interceptors used by the server interpreter. Custom interceptors should usually be added
  * using `addInterceptor`. That way, the custom interceptor is called after the built-in ones (such as logging,
  * metrics, exceptions), and before the decode failure handler. For even more flexibility, interceptors can be added to
  * the beginning or end of the interceptor stack, using `prependInterceptor` and `appendInterceptor`.
  *
  * The first interceptor in the interceptor stack is the one which is called first on request, and processes the
  * resulting response as the last one.
  *
  * Built-in interceptors can be customised or disabled using the dedicated methods.
  *
  * Once done, use [[options]] to obtain the server interpreter options objects, which can be passed to the server
  * interpreter.
  *
  * @param prependedInterceptors
  *   Additional interceptors, which will be called first on request / last on response, e.g. performing logging,
  *   metrics, or providing alternate responses.
  ...
  *   Whether to return 406 (not acceptable) if there's no body in the endpoint's outputs, which can satisfy the
  *   constraints from the `Accept` header.
  * @param additionalInterceptors
  *   Additional interceptors, which will be called before (on request) / after (on response) the `decodeFailureHandler`
  *   one, e.g. performing logging, metrics, or providing alternate responses.
  * @param decodeFailureHandler
  *   The decode failure handler, from which an interceptor will be created. Determines whether to respond when an input
  *   fails to decode.
  * @param appendedInterceptors
  *   Additional interceptors, which will be called last on request / first on response, e.g. handling decode failures,
  *   or providing alternate responses.
  */
case class CustomiseInterceptors[F[_], O](
  ...
  def appendInterceptor(i: Interceptor[F]): CustomiseInterceptors[F, O] =
    copy(appendedInterceptors = appendedInterceptors :+ i)
  ...
  /** Use the default exception, decode failure and reject handlers.
    ...
    *   return a 404 formatted using `errorMessageOutput` when the request was rejected by all endpoints, instead of
    *   propagating the rejection to the server library
    */
  def defaultHandlers(
