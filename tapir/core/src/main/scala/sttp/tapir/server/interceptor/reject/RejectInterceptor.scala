...
/** Specifies what should be done if decoding the request has failed for all endpoints, and multiple endpoints have been
  * interpreted (doesn't do anything when interpreting a single endpoint).
  *
...
object RejectInterceptor {
  ...
  /** When interpreting a single endpoint, disabling the reject interceptor, as returning a method mismatch only makes
    * sense when there are more endpoints
    */
  def disableWhenSingleEndpoint[F[_]](
      interceptors: List[Interceptor[F]],
      ses: List[ServerEndpoint[_, F]]
  ): List[Interceptor[F]] =
