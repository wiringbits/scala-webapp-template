...
private[play] class PlayRequestBody(serverOptions: PlayServerOptions)(implicit
  ...
  private def multiPartRequestToRawBody(
    ...
    bodyParser.apply(request).run(body()).flatMap {
      ...
      case Right(value) =>
        val dataParts: Seq[Future[Option[Part[Any]]]] =
          value.dataParts.flatMap { case (key, value: scala.collection.Seq[String]) =>
  ...
  private def playRequest(serverRequest: ServerRequest) =
    serverRequest.underlying.asInstanceOf[Request[Source[ByteString, Any]]]
}
