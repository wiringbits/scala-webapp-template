...
object TypoJsonb {
      ...
      case v: PgArray =>
        v.getArray match {
          ...
          case other =>
            Left(
              TypeDoesNotMatch(
                s"Expected one-dimensional array from JDBC to produce an array of TypoJsonb, got ${other.getClass.getName}"
              )
            )
        }
      case other =>
  ...
  implicit lazy val arrayToStatement: ToStatement[Array[TypoJsonb]] = ToStatement[Array[TypoJsonb]]((s, index, v) =>
    s.setArray(
      index,
      s.getConnection.createArrayOf(
        "jsonb",
        v.map(v => {
  ...
  implicit lazy val toStatement: ToStatement[TypoJsonb] = ToStatement[TypoJsonb]((s, index, v) =>
    s.setObject(
      index, {
        val obj = new PGobject
        obj.setType("jsonb")
