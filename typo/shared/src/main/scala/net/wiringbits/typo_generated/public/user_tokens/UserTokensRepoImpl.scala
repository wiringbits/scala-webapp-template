...
object UserTokensRepoImpl extends UserTokensRepo {
  override def delete(userTokenId: /* user-picked */ UUIDCustom)(implicit c: Connection): Boolean = {
    SQL"""delete from public.user_tokens where "user_token_id" = ${ParameterValue(userTokenId, null, implicitly[ToStatement[UUIDCustom]])}""".executeUpdate() > 0
  }
  ...
  override def insert(unsaved: UserTokensRow)(implicit c: Connection): UserTokensRow = {
    SQL"""insert into public.user_tokens("user_token_id", "token", "token_type", "created_at", "expires_at", "user_id")
          values (${ParameterValue(unsaved.userTokenId, null, implicitly[ToStatement[UUIDCustom]])}::uuid, ${ParameterValue(unsaved.token, null, ToStatement.stringToStatement)}, ${ParameterValue(unsaved.tokenType, null, ToStatement.stringToStatement)}, ${ParameterValue(unsaved.createdAt, null, implicitly[ToStatement[InstantCustom]])}::timestamptz, ${ParameterValue(unsaved.expiresAt, null, implicitly[ToStatement[InstantCustom]])}::timestamptz, ${ParameterValue(unsaved.userId, null, implicitly[ToStatement[UUIDCustom]])}::uuid)
  ...
  override def update(row: UserTokensRow)(implicit c: Connection): Boolean = {
    ...
    SQL"""update public.user_tokens
          set "token" = ${ParameterValue(row.token, null, ToStatement.stringToStatement)},
              ...
              "expires_at" = ${ParameterValue(row.expiresAt, null, implicitly[ToStatement[InstantCustom]])}::timestamptz,
