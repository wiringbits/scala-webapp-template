...
object UserLogsRepoImpl extends UserLogsRepo {
  override def delete(userLogId: /* user-picked */ UUIDCustom)(implicit c: Connection): Boolean = {
    SQL"""delete from public.user_logs where "user_log_id" = ${ParameterValue(userLogId, null, implicitly[ToStatement[UUIDCustom]])}""".executeUpdate() > 0
  }
  ...
  override def insert(unsaved: UserLogsRow)(implicit c: Connection): UserLogsRow = {
    SQL"""insert into public.user_logs("user_log_id", "user_id", "message", "created_at")
          values (${ParameterValue(unsaved.userLogId, null, implicitly[ToStatement[UUIDCustom]])}::uuid, ${ParameterValue(unsaved.userId, null, implicitly[ToStatement[UUIDCustom]])}::uuid, ${ParameterValue(unsaved.message, null, ToStatement.stringToStatement)}, ${ParameterValue(unsaved.createdAt, null, implicitly[ToStatement[InstantCustom]])}::timestamptz)
  ...
  override def insert(unsaved: UserLogsRowUnsaved)(implicit c: Connection): UserLogsRow = {
    val namedParameters = List(
      Some((NamedParameter("user_log_id", ParameterValue(unsaved.userLogId, null, implicitly[ToStatement[UUIDCustom]])), "::uuid")),
  ...
  override def selectByIds(userLogIds: Array[/* user-picked */ UUIDCustom])(implicit c: Connection, toStatement: ToStatement[Array[/* user-picked */ UUIDCustom]]): List[UserLogsRow] = {
    SQL"""select "user_log_id", "user_id", "message", "created_at"::text
          from public.user_logs
