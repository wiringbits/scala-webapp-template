...
object TypoUUID {
      ...
      case v: PgArray =>
        v.getArray match {
          ...
          case other =>
            Left(
              TypeDoesNotMatch(
                s"Expected one-dimensional array from JDBC to produce an array of TypoUUID, got ${other.getClass.getName}"
              )
            )
        }
      case other =>
  ...
  implicit lazy val arrayToStatement: ToStatement[Array[TypoUUID]] = ToStatement[Array[TypoUUID]]((s, index, v) =>
    s.setArray(index, s.getConnection.createArrayOf("uuid", v.map(v => v.value)))
  )
  ...
  implicit lazy val toStatement: ToStatement[TypoUUID] =
    ToStatement[TypoUUID]((s, index, v) => s.setObject(index, v.value))
