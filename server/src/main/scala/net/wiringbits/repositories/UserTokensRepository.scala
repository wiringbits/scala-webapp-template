...
class UserTokensRepository @Inject() (
  ...
  def find(userId: UUIDCustom, token: String): Future[Option[UserTokensRow]] = Future {
    database.withConnection { implicit conn =>
  ...
  def find(userId: UUIDCustom): Future[List[UserTokensRow]] = Future {
    database.withConnection { implicit conn =>
      UserTokensRepoImpl.select.where(_.userId === userId).orderBy(_.createdAt.desc).toList
    }
  }

  def delete(userTokenId: UUIDCustom, userId: UUIDCustom): Future[Unit] = Future {
    database.withConnection { implicit conn =>
