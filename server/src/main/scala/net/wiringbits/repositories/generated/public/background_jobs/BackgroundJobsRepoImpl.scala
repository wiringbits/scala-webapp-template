...
object BackgroundJobsRepoImpl extends BackgroundJobsRepo {
  override def delete(backgroundJobId: BackgroundJobsId)(implicit c: Connection): Boolean = {
    SQL"""delete from public.background_jobs where "background_job_id" = ${ParameterValue(backgroundJobId, null, BackgroundJobsId.toStatement)}""".executeUpdate() > 0
  }
  ...
  override def insert(unsaved: BackgroundJobsRowUnsaved)(implicit c: Connection): BackgroundJobsRow = {
    val namedParameters = List(
      Some((NamedParameter("background_job_id", ParameterValue(unsaved.backgroundJobId, null, BackgroundJobsId.toStatement)), "::uuid")),
      ...
      unsaved.executeAt match {
        case Defaulted.UseDefault => None
        case Defaulted.Provided(value) => Some((NamedParameter("execute_at", ParameterValue(value, null, TypoOffsetDateTime.toStatement)), "::timestamptz"))
      },
      ...
      unsaved.updatedAt match {
        case Defaulted.UseDefault => None
        case Defaulted.Provided(value) => Some((NamedParameter("updated_at", ParameterValue(value, null, TypoOffsetDateTime.toStatement)), "::timestamptz"))
      }
    ).flatten
    val quote = '"'.toString
  ...
  override def selectByIds(backgroundJobIds: Array[BackgroundJobsId])(implicit c: Connection): List[BackgroundJobsRow] = {
    SQL"""select "background_job_id", "type", "payload", "status", "status_details", "error_count", "execute_at"::text, "created_at"::text, "updated_at"::text
          from public.background_jobs
