...
object TokensDAO {
  ...
  def create(request: Token.CreateToken, expirationTime: FiniteDuration, clock: Clock)(implicit
      conn: Connection
  ): Unit = {
    val _ = SQL"""
        INSERT INTO tokens
        ...
        VALUES (
          ...
          ${Instant.now(clock)}::TIMESTAMP,
          ${request.expirationHour(expirationTime)(clock)}::TIMESTAMP,
  ...
  def delete(tokenId: UUID)(implicit conn: Connection): Unit = {
    val _ = SQL"""
         DELETE FROM tokens
