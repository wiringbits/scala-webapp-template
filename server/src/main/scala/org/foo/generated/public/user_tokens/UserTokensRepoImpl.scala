...
object UserTokensRepoImpl extends UserTokensRepo {
  override def delete(userTokenId: UserTokensId)(implicit c: Connection): Boolean = {
    SQL"""delete from public.user_tokens where "user_token_id" = ${ParameterValue(userTokenId, null, UserTokensId.toStatement)}""".executeUpdate() > 0
  }
